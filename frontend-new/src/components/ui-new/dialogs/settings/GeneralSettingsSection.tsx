...
import { useEngineModelConfig } from '@/hooks/useEngineModelConfig';
...
export function GeneralSettingsSection() {
  ...
  return (
    <>
      ...
      <SettingsCard
        title="模型配置"
        description="支持两种方式：使用 Anthropic 官方 API，或切换到 Claude Code 会员登录。"
      >
        {modelConfigError && (
        ...
        {modelConfigSuccess && (
          <div className="bg-success/10 border border-success/50 rounded-sm p-3 text-sm text-success">
            {modelConfigSuccess}
          </div>
        )}
        ...
        {modelConfigLoading ? (
          ...
          <>
            <SettingsField
              label="API 地址"
              description="Anthropic 兼容接口地址。"
            >
              <SettingsInput
                value={modelBaseUrl}
                onChange={setModelBaseUrl}
            ...
            <SettingsField
              ...
              description={
                hasExistingApiKey
                  ? '当前已存在 API Key，留空不会覆盖。'
                  : '可选，留空则走环境变量或 Claude 登录态。'
              }
            >
              <input
                type="password"
            ...
            <SettingsField
              label="模型"
              description="会同步应用到 Agent、GM、Verifier 模型槽位。"
            >
              <SettingsInput
                value={modelName}
            ...
            {hasMixedModels && (
              <p className="text-sm text-low">
                当前引擎配置里存在多个模型。保存后会统一为上方模型。
              </p>
            )}

            <p className="text-sm text-low">
              会员登录模式会清空 API Key，并使用官方地址 `https://api.anthropic.com`。请先在终端完成 `claude` 登录。
            </p>
            ...
            <div className="flex gap-2">
              <PrimaryButton
                variant="tertiary"
                value="恢复默认 Claude"
                onClick={restoreDefaultClaude}
                disabled={modelConfigSaving}
              />
              <PrimaryButton
                variant="tertiary"
                value="切换到会员登录"
                onClick={useClaudeCodeMembership}
                disabled={modelConfigSaving}
              />
              <PrimaryButton
                value="保存自定义配置"
                ...
                actionIcon={modelConfigSaving ? 'spinner' : undefined}
              />
            </div>
          </>
        )}
