  ...
  return (
    <div
      className={cn(
        'flex items-center',
        isMobile
          ? 'gap-half overflow-x-auto pb-1 flex-nowrap [scrollbar-width:none] [&::-webkit-scrollbar]:hidden'
          : 'gap-base flex-wrap'
      )}
    >
      <InputField
        ...
        className={cn(isMobile ? 'min-w-[220px] shrink-0' : 'min-w-[200px]')}
      />
      ...
      <button
        ...
        className={cn(
          'shrink-0 flex items-center justify-center p-half rounded-sm',
      ...
      {hasActiveFilters &&
        (isMobile ? (
          <button
            ...
            className="shrink-0 rounded-sm p-half text-brand hover:text-brand hover:bg-secondary transition-colors"
            aria-label={t('kanban.clearFilters', 'Clear all')}
          >
